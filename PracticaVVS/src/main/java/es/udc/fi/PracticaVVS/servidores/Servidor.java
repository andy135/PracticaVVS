...
public class Servidor implements InterfazServidor{
	...
	public Servidor(String nombre, Token specialToken) {
		super();
		this.nombre = nombre;
	...
	public void agregar(Contenido contenido, Token token) throws UnexistingTokenException {
		if(token.equals(specialToken)){ //Comprueba que el token sea el correcto
			contenidos.add(contenido);
		}else throw new UnexistingTokenException();
	}
	...
	public void eliminar(Contenido contenido, Token token) 
		...
		if(token.equals(specialToken)){ //Comprueba que el token sea el correcto
			if(contenidos.contains(contenido)){  //Comprueba que el contenido exista
	...
	public List<Contenido> buscar(String subcadena, Token token) throws UnexistingTokenException, CadenaErroneaException {
		...
		if(activeTokens.contains(token)){ //Comprobamos que el token existe
			...
			if(token.getCount()<=0){//Añadimos publicidad de ser necesario
